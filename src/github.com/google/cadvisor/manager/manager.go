...
// Added - containerHousekeepingInterval time.Duration
func New(memoryCache *memory.InMemoryCache, sysfs sysfs.SysFs, maxHousekeepingInterval time.Duration, allowDynamicHousekeeping bool, ignoreMetricsSet container.MetricSet, collectorHttpClient *http.Client, containerHousekeepingInterval time.Duration) (Manager, error) {
	if memoryCache == nil {
	...
	newManager := &manager{
		...
		containerHousekeepingInterval: containerHousekeepingInterval,		//Added parameter - interval for collecting container metrics.
	}
...
type manager struct {
	...
	containerHousekeepingInterval time.Duration			//Newly Added containerHouseKeeping.
}
...
func (m *manager) createContainerLocked(containerName string, watchSource watcher.ContainerWatchSource) error {
	...
	//Newly Added m.containerHousekeepingInterval
	...
	cont, err := newContainerData(containerName, m.memoryCache, handler, logUsage, collectorManager, m.maxHousekeepingInterval, m.allowDynamicHousekeeping, m.containerHousekeepingInterval)
	if err != nil {
		return err
	}
